...
class NewsFile(models.Model):
    file = models.FileField(upload_to='upload/%Y/%m/%d',default=None,blank = True,validators=[FileExtensionValidator(allowed_extensions=['jpg','png','gif','pdf'])])
...
class DocumentF(models.Model):
    ...
    target_departament = models.CharField(max_length=3, choices=DEPARTAMENTS,default='ALL')
    target_location = models.CharField(max_length=3, choices=COMPANY_LOCATIONS,default='ALL')
    category = models.ForeignKey('news.KnowledgeCategory',on_delete=models.PROTECT,default=2,related_name="docs")
...
class DocFile(models.Model):
    ...
    target_departament = models.CharField(max_length=3, choices=DEPARTAMENTS,default='ALL')
    target_location = models.CharField(max_length=3, choices=COMPANY_LOCATIONS,default='ALL')
    category = models.ForeignKey('news.KnowledgeCategory',on_delete=models.PROTECT,default=2,related_name="files")


class DocQuestion(models.Model):
    title =  models.CharField(max_length=200)
    body = models.TextField(max_length=5000)
    answer = models.TextField(max_length=5000,default=None,blank=True,null=True)
    target_departament = models.CharField(max_length=3, choices=DEPARTAMENTS,default='ALL')
    target_location = models.CharField(max_length=3, choices=COMPANY_LOCATIONS,default='ALL')
    date_created = models.DateTimeField(default=timezone.now)
    category = models.ForeignKey('news.KnowledgeCategory',on_delete=models.PROTECT,default=2,related_name="questions")
