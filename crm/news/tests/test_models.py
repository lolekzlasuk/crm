...
class NewsModelTest(TestCase):
    ...
    def test_news_date_created_label(self):
        field_label = self.test_news._meta.get_field('date_created').verbose_name
    ...
    def test_news_published_date_label(self):
        field_label = self.test_news._meta.get_field('published_date').verbose_name
    ...
    def test_news_target_departament_label(self):
        field_label = self.test_news._meta.get_field('target_departament').verbose_name
        self.assertEqual(field_label, 'target departament')

    def test_news_target_location_label(self):
        field_label = self.test_news._meta.get_field('target_location').verbose_name
    ...
    def test_news_target_location_max_length(self):
        max_length= self.test_news._meta.get_field('target_location').max_length
        self.assertEqual(max_length, 3)

    def test_news_target_departament_max_length(self):
        max_length= self.test_news._meta.get_field('target_departament').max_length
        self.assertEqual(max_length, 3)
...
# target_location
    ...
    def setUpTestData(cls):
        ...
        simple_file = SimpleUploadedFile("testfile1.pdf", b"these are the file contents!")
        ...
        simple_file_image = SimpleUploadedFile(name = "testimage.jpg",  content=open(test_image_path, 'rb').read(), content_type='image/jpeg')
    ...
    def test_news_file_extension_label(self):
        field_label = self.test_news_file._meta.get_field('extension').verbose_name
        self.assertEqual(field_label, 'extension')

    def test_news_file_miniature_label(self):
        field_label = self.test_news_file._meta.get_field('miniature').verbose_name
    ...
    def test_news_file_object_name(self):
        ...
        self.assertEqual(self.test_news_file.file.name.split("/")[-1], expected_object_name)
        self.assertEqual(self.test_news_file.file.name.split("/")[-1][-4:], expected_object_ext)
    ...
    def test_news_file_miniature_path(self):
        ...
        self.assertEqual(self.test_news_file_2.miniature, expected_miniature_path)
...
class NotificationModelTest(TestCase):
    ...
    def test_notification_body_label(self):
        field_label = self.test_notification._meta.get_field('body').verbose_name
        self.assertEqual(field_label, 'body')

    def test_notification_title_label(self):
        field_label = self.test_notification._meta.get_field('title').verbose_name
        self.assertEqual(field_label, 'title')

    def test_notification_date_created_label(self):
        field_label = self.test_notification._meta.get_field('date_created').verbose_name
        self.assertEqual(field_label, 'date created')
...
class NotificationReadFlagModelTest(TestCase):
    ...
    def test_notificationreadflag_read_label(self):
        field_label = self.test_notificationreadflag._meta.get_field('read').verbose_name
        self.assertEqual(field_label, 'read')

    def test_notificationreadflag_user_label(self):
        field_label = self.test_notificationreadflag._meta.get_field('user').verbose_name
        self.assertEqual(field_label, 'user')

    def test_notificationreadflag_notification_label(self):
        field_label = self.test_notificationreadflag._meta.get_field('notification').verbose_name
        self.assertEqual(field_label, 'notification')

    def test_notificationreadflag_object_name(self):
        expected_object_name = '{0} ({1}) '.format(self.test_notificationreadflag.user.userprofile, self.test_notificationreadflag.notification)
        self.assertEqual(str(self.test_notificationreadflag), expected_object_name)
...
# category
    ...
    def test_docfile_date_created_label(self):
        field_label = self.test_docfile._meta.get_field('date_created').verbose_name
        self.assertEqual(field_label, 'date created')
    ...
    def test_docfile_target_departament_label(self):
        field_label = self.test_docfile._meta.get_field('target_departament').verbose_name
        self.assertEqual(field_label, 'target departament')

    def test_docfile_target_location_label(self):
        field_label = self.test_docfile._meta.get_field('target_location').verbose_name
        self.assertEqual(field_label, 'target location')

    def test_docfile_category_label(self):
        field_label = self.test_docfile._meta.get_field('category').verbose_name
    ...
    def test_docfile_target_location_max_length(self):
        max_length= self.test_docfile._meta.get_field('target_location').max_length
        self.assertEqual(max_length, 3)

    def test_docfile_target_departament_max_length(self):
        max_length= self.test_docfile._meta.get_field('target_departament').max_length
        self.assertEqual(max_length, 3)
...
# category
    ...
    def test_documentf_author_label(self):
        field_label = self.test_documentf._meta.get_field('author').verbose_name
        self.assertEqual(field_label, 'author')

    def test_documentf_date_created_label(self):
        field_label = self.test_documentf._meta.get_field('date_created').verbose_name
        self.assertEqual(field_label, 'date created')

    def test_documentf_target_departament_label(self):
        field_label = self.test_documentf._meta.get_field('target_departament').verbose_name
        self.assertEqual(field_label, 'target departament')

    def test_documentf_target_location_label(self):
        field_label = self.test_documentf._meta.get_field('target_location').verbose_name
        self.assertEqual(field_label, 'target location')

    def test_documentf_category_label(self):
        field_label = self.test_documentf._meta.get_field('category').verbose_name
        self.assertEqual(field_label, 'category')
    ...
    def test_documentf_target_location_max_length(self):
        max_length= self.test_documentf._meta.get_field('target_location').max_length
        self.assertEqual(max_length, 3)

    def test_documentf_target_departament_max_length(self):
        max_length= self.test_documentf._meta.get_field('target_departament').max_length
        self.assertEqual(max_length, 3)
...
# category
    ...
    def test_docquestion_title_label(self):
        field_label = self.test_docquestion._meta.get_field('title').verbose_name
        self.assertEqual(field_label, 'title')

    def test_docquestion_body_label(self):
        field_label = self.test_docquestion._meta.get_field('body').verbose_name
        self.assertEqual(field_label, 'body')

    def test_docquestion_answer_label(self):
        field_label = self.test_docquestion._meta.get_field('answer').verbose_name
        self.assertEqual(field_label, 'answer')

    def test_docquestion_date_created_label(self):
        field_label = self.test_docquestion._meta.get_field('date_created').verbose_name
        self.assertEqual(field_label, 'date created')

    def test_docquestion_target_departament_label(self):
        field_label = self.test_docquestion._meta.get_field('target_departament').verbose_name
        self.assertEqual(field_label, 'target departament')

    def test_docquestion_target_location_label(self):
        field_label = self.test_docquestion._meta.get_field('target_location').verbose_name
        self.assertEqual(field_label, 'target location')

    def test_docquestion_category_label(self):
        field_label = self.test_docquestion._meta.get_field('category').verbose_name
        self.assertEqual(field_label, 'category')
    ...
    def test_docquestion_target_location_max_length(self):
        max_length= self.test_docquestion._meta.get_field('target_location').max_length
        self.assertEqual(max_length, 3)

    def test_docquestion_target_departament_max_length(self):
        max_length= self.test_docquestion._meta.get_field('target_departament').max_length
        self.assertEqual(max_length, 3)
...
class UserQuestionModelTest(TestCase):
    ...
    def test_docquestion_author_label(self):
        field_label = self.test_docquestion._meta.get_field('author').verbose_name
        self.assertEqual(field_label, 'author')
